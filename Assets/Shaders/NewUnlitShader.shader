    ...
    SubShader
    {
        ...
        void Unity_Flipbook_float(float2 UV, float Width, float Height, float Tile, float2 Invert, out float2 Out)
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2  vectorForvard2D = mul(UNITY_MATRIX_M, float4(0,0,1,0)).xz;

                float angle = dot(vectorForvard2D, i.cameraDir);

                float angleRad = acos(angle);

                float anglNormolized = angleRad / 3.1415;

                float3 crossProduct = cross(float3(vectorForvard2D.x,0,vectorForvard2D.y),float3(i.cameraDir.x,0,i.cameraDir.y));

                if(dot(crossProduct, float3(0,1,0)) <0)
                    anglNormolized = -anglNormolized;

                float finalAngle = (anglNormolized +1)/ 2;
